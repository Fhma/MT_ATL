				...
				if (match.getPattern().getLevel() == level) {
					...
					if (res.equals(IMutationChecker.NOT_MUTATABLE))
						throw new IllegalArgumentException(
								"Property [" + property.getName() + "] is unable to be mutated");

					String operatorName = getConcreteOperator(mu_action) + "_" + property.getContainerName() + "_"
							+ property.getName();
					...
					if (do_ != null && res.equals(IMutationChecker.VALID)) {
						...
						Object new_value = property.getModel().getPropertyGetter().invoke(property.getContainer(),
								property.getName());
					...
					if (valid_mutant == null
							|| (valid_mutant != null && valid_mutant.equals(IMutationChecker.INVALID))) {
						module.getOperatorsMatrix().getValue(operatorName).add(INVALID_MUTATION);
					}
				}
			}
